...
from bot.services.assemblyai import AssemblyAIError, transcribe_file_assemblyai
...
async def _process_job(bot: Bot, job_id: int) -> None:
    ...
    try:
        ...
        await _mark_failed(job_id, str(exc))
        logger.exception("AssemblyAI failed for media job id=%s", job_id)
        await bot.send_message(job.chat_id, f"Задача #{job.id}: AssemblyAI не смог расшифровать запись: {escape(str(exc), quote=False)}")
...
async def _transcribe_with_provider(local_path: Path, provider: str) -> str:
    if provider == "assemblyai":
        return await transcribe_file_assemblyai(local_path)
