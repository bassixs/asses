...
- Добавлен `.txt` файл расшифровки:
  - полный транскрипт в формате диалога `[тайминг] ведущий/участник: ...`;
  - роли всегда приводятся к двум значениям: `ведущий` и `участник`;
  - строки дополнительно очищаются YandexGPT от очевидных ошибок распознавания без изменения смысла;
...
12. Worker создает `.txt` файл с полным транскриптом в формате `[тайминг] ведущий/участник: ...`.
13. Бот отвечает:
