...
- Добавлен `.txt` файл расшифровки:
  - полный транскрипт SpeechKit;
  - ролевой блок, сформированный YandexGPT;
...
10. Транскрипт сохраняется в таблицу `interview_records`.
11. Worker создает `.txt` файл с полным транскриптом и ролевой разметкой.
12. Бот отвечает:
...
14. `bot/services/assessment.py` формирует system prompt и вызывает YandexGPT.
15. YandexGPT возвращает JSON по компетенциям.
16. JSON валидируется Pydantic-моделью `AssessmentReport`.
17. Результат сохраняется в таблицу `assessment_results`.
18. Бот отправляет HR-у отчет.
