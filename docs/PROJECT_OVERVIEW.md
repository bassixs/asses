...
Текущая боевая связка (2026-06-14): **всё на AI Tunnel**, разные модели под задачи.

```text
STT:                 AI Tunnel Whisper (whisper-large-v3-turbo, verbose_json — с таймкодами)
Разметка ролей:      AI Tunnel gpt-5-mini, JSON-режим (3 голоса без диаризации — нужна сильная модель)
Анализ/оценка блокнота: AI Tunnel gpt-4o-mini, JSON-режим
Рекомендации/ИПР:    AI Tunnel gpt-4o-mini
PDF:                 LibreOffice headless на сервере
```

NeuroAPI и YandexGPT остаются в коде как fallback-провайдеры (переключаются через `.env`).
История: анализ был на NeuroAPI `deepseek-v4-flash` (плохо держал JSON) → перешли на gpt-4o-mini.
Разметку ролей сначала тоже перевели на gpt-4o-mini, но она регрессировала (путала 3 голоса) —
подняли до **gpt-5-mini**, после чего роли стали стабильно корректными.
...
bot/services/observer_notebook.py   анализ/заполнение Excel-блокнота (батчи, терпимый JSON, таймкоды evidence)
bot/services/development_advice.py   LLM: отчётные формулировки, рекомендации, объект ipr (70/20/10)
bot/services/competency_content.py   матчинг компетенции к библиотеке заказчика
bot/services/report_pptx.py          рендер отчёта-презентации (PPTX) по образцу заказчика
bot/services/pdf_export.py           конвертация DOCX/PPTX в PDF через LibreOffice
bot/resources/competency_library.json  курированная библиотека (литература/курсы/задания ×6 компетенций)
