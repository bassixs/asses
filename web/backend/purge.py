...
async def delete_exercises(session: AsyncSession, exercise_ids: list[int]) -> dict[str, int]:
    """Remove exercises and everything recorded against them."""
    if not exercise_ids:
        return {"exercises": 0, "results": 0}
    ...
    return {"exercises": len(exercise_ids), "results": processed}


async def delete_participants(session: AsyncSession, participant_ids: list[int]) -> dict[str, int]:
    """Remove participants, their exercises, reports and development plans."""
    if not participant_ids:
        return {"participants": 0, "exercises": 0, "results": 0}
    ...
    return {"participants": len(participant_ids), **stats}


async def delete_center(session: AsyncSession, center_id: int) -> dict[str, int]:
